...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! start input parameters !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  dt = 0.05 ! time step, should obey Courant limit: dt < dz/vmax
  ...
  ky = 0.3 ! y-wavenumber
  omega_n = 3.0 ! omega_n = L_z/L_n
  omega_Ti = 30.0 ! omega_Ti = L_z/L_Ti
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! end input parameters !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  DO t = 1, Ntsteps
    ...
    Ftemp(1:Nz,1:Nv) = F(1:Nz,1:Nv) + dt * k1(1:Nz,1:Nv) * 0.3333333333 ! can't remember why the 0.33 is there
...
CONTAINS
  ...
  SUBROUTINE Phicalc(Phi,F) ! here f(x,v) is integrated over v to get f(x) which is the density distribution and essentially electrostatic potential if the charge per electron is 1.  
    INTEGER :: k, l
  ...
  SUBROUTINE kieval(ki,F)! This is where the drift kinetic equations are implimented on slide 12 of lecture 6. It moved F or f(x,v) one step forward. what is the differnece between F, k1 and k23, I don't know. F is previous f(x,v)
    INTEGER :: k, l
    ...
    DO k = 1, Nz
      DO l = 1, Nv
        ki(k,l) = - F0(l) * ((omega_n + omega_Ti * (vpar(l) * vpar(l) - 0.5)) * i * ky * Phi(k) + &
          ...
          Di * (F(k+1,l) - 2 * F(k,l) + F(k-1,l)) * inv_dz2 !stencil [1 -2 1] is used for the double differential and [-1 0 1] for the double differential
      END DO
    END DO
  END SUBROUTINE
